...
SigSpBox <- eDNAxEnvDataSatClean %>% 
  filter(Species %in% c("Aphanomyces astaci", "Aureococcus anophagefferens", "Heterosigma akashiwo", "Thecamonas trahens", "Chaetoceros socialis", "Lithodesmium intricatum", "Cyphoderia ampulla", "Bathycoccus prasinos", "Citharichthys sordidus", "Clytia gregaria", "Paramoeba branchiphila", "Vannella sp.", "Parvamoeba rugata", "Chrysochromulina sp. NIES-1333", "Thermocyclops inversus", "Pennaria disticha", "Stephanopyxis turris")) # Species I actually want to run this on, NA values removed
...
for (i in 1:length(LinearSplit)) {
  ...
  box <- boxTidwell(Present ~ SatPct, other.x = ~ year, data = LinearSplit[[i]])
...
To kill enter in terminal `while true; do pkill -9 say; done`

```{r, eval = F}
for (i in 1:length(SpeciesMod2)) {
  x <- SpeciesMod2[i]
  system(paste("say", x))
}
```
