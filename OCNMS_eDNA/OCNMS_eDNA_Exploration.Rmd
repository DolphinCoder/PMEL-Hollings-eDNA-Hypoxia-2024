...
df_out1 <- PCR_reps_combine(allReads)
```
...
```{r}
id_cols <- colnames(allReads) # Pull out column names
id_cols <- id_cols[! id_cols %in% c("X", "ESV", "sequence", "pctMatch", "nReads")] # Remove ESV + pctMatch + nreads

allReads_species <- allReads %>% 
  group_by_at(id_cols) %>% # group_by_at can take a vector
  summarize(TotalnReads = sum(nReads), avgpctMatch = mean(pctMatch)) %>% 
  relocate(TotalnReads, .after = SampleId)
