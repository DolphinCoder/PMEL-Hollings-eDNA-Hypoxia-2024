...
CTD <- read.csv(here(wd, "OCNMS_CTD_TH042_CleanData.csv")) %>% 
  mutate(date = as.POSIXct(date), year = as.factor(year), source = as.factor(source))
Mooring <- read.csv(here(wd, "OCNMS_Mooring_CleanData.csv")) %>% 
  mutate(date = as.POSIXct(date), year = as.factor(year), source = as.factor(source))

EnvData1full <- rbind(CTD, Mooring) %>% 
  select(-X) %>% 
  mutate(date = as.POSIXct(date), year = as.factor(year), source = as.factor(source))

EnvData1 <- EnvData1full 
```
...
ggsave(here(pltpath, "CombinedOxygenSeries_crop.png"), height = 2000, width = 3000, units = "px")
...
ggsave(here(pltpath, "CombinedTemperatureSeries_crop.png"), height = 2000, width = 3000, units = "px")
```
