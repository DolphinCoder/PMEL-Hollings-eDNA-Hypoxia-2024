...
```{r}
ggplot(data = MODIS_sst %>% filter(month(date) == 06), aes(x = as.Date(date), y = SST_C, color = Latitude)) +
  geom_point(shape = 1) +
  facet_wrap(facets = vars(year(date)), scales = "free_x", ncol = 1) +
  theme_bw()

ggplot(data = MODIS_sst %>% filter(month(date) == 06), aes(x = as.Date(date), y = SST_C, color = Longitude)) +
  geom_point(shape = 1) +
  facet_wrap(facets = vars(year(date)), scales = "free_x", ncol = 1) +
  theme_bw()
```
