...
TH042$time <- as.POSIXct(TH042$time, tryFormats = c("%e-%b-%Y %H:%M:%S"))
TH042 <- TH042 %>%
...
```{r}
TH042 <- TH042 %>% 
  rename(DO = oxy_mg, temperature = temp, salinity = sal)
```
...
histogram_fill(TH042, dens, 0.1, fill = year)
...
```{r}
scatterplt(TH042, dens, salinity, alpha = 0.1)
scatterplt_fill(TH042, dens, salinity, color = year)
```
