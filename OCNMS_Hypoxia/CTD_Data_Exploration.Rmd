...
scatterplt_fill(OCNMS_OME_CTD, depth, DO, alpha = 0.5, color = station_id) +
  scale_color_manual(values = c("gray80", "blue"))
ggsave(here(pltpath, "Depth_DO_Scatterplot_CTD.png"))
```
...
histogram_fill(OCNMS_OME_CTD %>% filter(year %in% c(2021, 2022, 2023)), 
               ...
               fill = station_id) +
  facet_wrap(facets = vars(year)) +
  scale_x_continuous(limits = c(30,46), breaks = seq(from = 30, to = 46, by = 2)) +
...
write.csv(OCNMS_OME_CTD_TH042_extended_exp, file = here("OCNMS_Hypoxia", "Outputs", "OCNMS_CTD_TH042ext_CleanData.csv"))
```
