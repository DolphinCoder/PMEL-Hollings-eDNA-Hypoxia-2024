...
# cruise
# comment
datenum <- c(lst[[1]]) # datenums as vector
metadata <- lst[[38]]
dates <- as.POSIXct((datenum - 719529)*86400, origin = "1970-01-01", tz = "UTC") # subtract to get origin = January 0, 0000 (this is a made up date so as.POSIXct won't handle it natively), need to multiply by seconds in a day to go from datenum (days since made up Matlab epoch) to POSIXct (seconds since Unix epoch). Just kinda hoping they were using UTC here.
print(length(datenum) == length(dates))
